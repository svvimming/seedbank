...
class Bicho extends Component {
  ...
  moveCentre = () => {
    ...
    for (var i=0; i<this.nodes; i++){
      this.nodeX[i] = this.x+Math.cos(this.rotAngle*this.rotrate)*this.radius +Math.sin(this.angle[i]*this.rotrate);
      this.nodeY[i] = this.y+Math.sin(this.rotAngle*this.rotrate)*this.radius +Math.sin(this.angle[i]*this.rotrate);
  ...
  setup = (p5, canvasParentRef) => {
    ...
    p5.createCanvas(this.canvas_width, this.canvas_height).parent(canvasParentRef); // use parent to render canvas in this ref (without that p5 render this canvas outside your component)
    p5.noStroke();
    p5.frameRate(30);
       ...
       for (var j=1; j < this.nodes+1; j++){
         p5.curveVertex(
           ...
           (
             ...
             + this.json[j+i].x
             + offset
             + 0.4*100*Math.sin(this.nodeX[j]*(rate+Math.PI/this.nodes)*this.rotrate)
                  ),
           //y
           (
             this.nodeY[j]
             + this.json[j+i].y
             + offset
             + 0.4*100*Math.cos(this.nodeY[j]*(rate+Math.PI/this.nodes)*this.rotrate)
                  )
         );
       }
  ...
  render() {
    return (
    <div>
      <Sketch style = {{position: "absolute", top: this.props.topPos, left: this.props.leftPos, zIndex: -100,}} setup={this.setup} draw={this.draw} />
      </div>
    );
  }
}
...
class Doodle extends Component {
  ...
  varyColor = (mx, my) => {
      var h = (Math.abs(this.hue + Math.floor((mx-this.dummyX)*(360/this.canvas_width))) % 360);
      var g = Math.min(Math.abs(this.gray + Math.floor((my-this.dummyY)*(100/this.canvas_height))), 100);
  ...
  animateX = (i) => {
    var x = this.motion*20*Math.sin(this.spin*i*Math.PI/this.points.length);
    return x;
  }

  animateY = (i) => {
    var y = this.motion*20*Math.cos(this.spin*i*Math.PI/this.points.length);
    return y;
  }
  ...
  mousePressed = (p5) => {
      ...
      } else if (!this.state.drawing && this.state.coloring && !this.state.submitting) {
        this.setState({
          drawing : false,
          coloring : false,
          submitting: true,
  ...
  draw = p5 => {
    ...
    if(this.state.submitting){
      this.spin = 10*Math.sin((this.incA+this.incB)*Math.PI/this.points.length);
    ...
    p5.beginShape();
    for (var i = 0; i < this.points.length; i++) {
      p5.vertex(this.points[i].x + this.animateX(i), this.points[i].y + this.animateY(i));
    }
    p5.endShape();
  ...
  render() {
    return <Sketch style = {{position: "absolute", top: this.props.topPos, left: this.props.leftPos}} setup={this.setup} draw={this.draw} mousePressed={this.mousePressed} mouseDragged={this.mouseDragged} mouseReleased={this.mouseReleased}/>;
  }
}
